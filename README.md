...
当前版本 **v0.2.15 / code19**：图片附件显示缩略图，可点击全屏缩放查看，支持登录鉴权、失败重试和后台解码。保留流式滚动稳定性、大型会话懒加载、表格分页、会话删除、运行中补充及服务端提供的 MCP 和原生最高思考档位。修复明细见 [CHANGELOG.md](CHANGELOG.md) 与 [VALIDATION.md](VALIDATION.md)。
...
- 待批准与待回答请求显示在输入区，按顺序处理；批准展示命令／请求、Agent 选项，并可填写拒绝原因。支持单选、多选、文本、自定义回答、可选问题跳过及取消回答，按官方 `user_input_response` 提交。
- 省略 `can_approve`／`can_respond` 时按官方约定允许处理 pending 请求，显式 false、已结束或外部渠道会话不可操作。控制回执核对会话／运行任务／类型；服务端已确认的结果不会被迟到 pending 快照恢复成可点击按钮，失败可重试，断线不自动重发决定。
- 展示文本、可折叠思考／工具详情、错误、通知及未知块；同一会话最多展开一个工具，展开另一个时自动折叠前一个，支持复制正文和返回最新消息；阅读旧消息时不强制跳底。流式和最终文本均使用 Markwon，不使用 WebView；未引入图片插件，因而不会加载远程 Markdown 图片。
- 回复等待期间使用 `dataSync` 前台服务。服务使用第二条**只读**订阅监控完成状态，不发送或重发用户消息。
- Android 13+ 在用户从可见界面发送时请求通知权限。
- 通知只保留系统左侧身份图标，移除右侧重复 logo。应用图标提供官方紫色 Memoh 的自适应及五档密度 PNG 资源；通知 small icon 使用同一标识的单色轮廓和品牌色，最终颜色由系统通知样式决定。
- 输入区“＋”通过系统文件选择器添加文档或图片，可多选、移除、重试读取及仅发送附件；每条最多 10 个，合计 8 MB。读取后点击发送，附件按官方格式随消息一并提交。
...
- 运行中可输入文字并点击“补充到当前回复”。内容先加密暂存，区分排队、已插入、拒绝和状态未知；断线不自动重发，失败可恢复到输入框。需要配套服务端补丁，支持原生运行时、配套 Pi ACP 和 Codex ACP；未支持的 ACP 适配器会明确拒绝。普通草稿仍只在内存中。
- 服务端已接收的任务独立于 App 连接执行。服务端配套 bridge 修复约 30 分钟强制断开 ACP 进程的问题；等待审批时仍需用户操作，执行设备须持续在线。
...
- 回答表单和拒绝原因仅保留在当前页面内存中；离开会话或结束进程后需要重新填写，未确认的回答不会自动重发。
- 成功入 OkHttp 队列不等于服务端已接纳。网络中断导致结果未知时必须核对原会话，客户端不会重放 prompt；监听暂停也不停止远程工作。
- 未实现历史向前分页、会话改名、编辑/重试和多并发待处理回复。
- 图片消息支持受保护媒体、Base64 图片与 HTTPS 图片地址的缩略图和全屏预览；暂未实现通用文件管理、文件夹上传、非图片附件预览或保存到系统相册。图片下载上限 24 MB，后台缩略解码，GIF 当前显示静态帧；Markdown 远程图片插件仍未启用。文件是否能被模型直接理解由模型及 Agent 能力决定。附件草稿只在进程内保存，退出应用进程或登出会清除；每条上传最多 10 个文件、合计 8 MB。
