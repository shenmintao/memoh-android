...
当前发布版本：`0.2.1`（versionCode 5）。修复明细见 [CHANGELOG.md](CHANGELOG.md)。
...
- 下拉通知卡片显示官方紫色 Memoh logo，状态栏使用同一 logo 的单色轮廓。
...
- 模型按钮支持搜索和选择普通聊天模型、直接 Agent 模型和 ACP 动态模型。ACP 切换由服务器确认，失败后可刷新重试；设备／模型选择仅作用于当前会话，不更改机器人的全局默认设置。
