...
当前发布版本：`0.2.6`（versionCode 10）。修复明细见 [CHANGELOG.md](CHANGELOG.md)。
...
- 未实现历史向前分页、会话删除/改名、编辑/重试和多并发待处理回复。
- 附件目前是聊天附件上传，未实现文件管理器、文件夹上传和消息附件下载／预览。文件是否能被模型直接理解由模型及 Agent 能力决定。附件草稿只在进程内保存，退出应用进程或登出会清除；为控制移动端内存和 WebSocket 帧体积，每条最多 10 个文件、合计 8 MB。
